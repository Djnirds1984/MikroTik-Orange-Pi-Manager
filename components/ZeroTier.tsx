    ...
    if (status === 'error' || !data) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-slate-800 rounded-lg border border-red-700 p-6 text-center">
                ...
                <p className="mt-2 text-slate-400 text-sm">{errorMessage || 'Could not parse data from the server.'}</p>
                <button onClick={fetchData} className="mt-6 px-4 py-2 bg-red-600/50 hover:bg-red-500/50 rounded-lg font-semibold">
                    Try Again
                </button>
            </div>
        );
                                ...
                                {data.networks.length > 0 ? data.networks.map(net => (
                                    <tr key={net.nwid} className="border-b border-slate-700 last:border-b-0 hover:bg-slate-700/50">
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            <button onClick={() => handleLeave(net.nwid)} disabled={isSubmitting} className="p-2 text-slate-400 hover:text-red-500 rounded-md disabled:opacity-50" title="Leave Network">

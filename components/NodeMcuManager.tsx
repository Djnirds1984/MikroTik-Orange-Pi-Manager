                            ...
                            {nodeMcuDevices.length > 0 ? nodeMcuDevices.map(device => (
                                <tr key={device.id} className="border-b border-slate-200 dark:border-slate-700 last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="px-6 py-4 text-right space-x-2">
                                        <button onClick={() => handleAction(device.address, 'reboot')} className="px-3 py-1 text-xs bg-slate-200 hover:bg-slate-300 dark:bg-slate-600 dark:hover:bg-slate-500 rounded-md font-semibold text-slate-700 dark:text-white" title="Not implemented">
                                            Reboot
                                        </button>
                                        <button onClick={() => handleAction(device.address, 'settings')} className="px-3 py-1 text-xs bg-slate-200 hover:bg-slate-300 dark:bg-slate-600 dark:hover:bg-slate-500 rounded-md font-semibold text-slate-700 dark:text-white" title="Not implemented">
                                            Settings
                                        </button>
                                    </td>
                                </tr>

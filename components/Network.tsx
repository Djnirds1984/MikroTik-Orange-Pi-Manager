        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-slate-800 rounded-lg shadow-xl w-full max-w-lg border border-slate-700">
                <form onSubmit={handleSubmit}>
                    <div className="p-6">
                        <h3 className="text-xl font-bold text-orange-400 mb-4">Add New VLAN</h3>
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-slate-300">VLAN Name</label>
                                <input type="text" name="name" id="name" value={vlanData.name} onChange={handleChange} required className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-orange-500" placeholder="e.g., vlan10-guests" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label htmlFor="vlan-id" className="block text-sm font-medium text-slate-300">VLAN ID</label>
                                    <input type="number" name="vlan-id" id="vlan-id" value={vlanData['vlan-id']} onChange={handleChange} min="1" max="4094" required className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white" />
                                </div>
                                <div>
                                    <label htmlFor="interface" className="block text-sm font-medium text-slate-300">Parent Interface</label>
                                    <select name="interface" id="interface" value={vlanData.interface} onChange={handleChange} required className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-orange-500">
    ...
    if (!selectedRouter) {
        return (
            <div className="flex flex-col items-center justify-center h-96 text-center bg-slate-800 rounded-lg border border-slate-700">
                ...
                <h2 className="text-2xl font-bold text-slate-200">Network Management</h2>
                <p className="mt-2 text-slate-400">Please select a router to manage its network settings.</p>
            </div>
        );
    }
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-64">
                <Loader />
                <p className="mt-4 text-orange-400">Fetching network data from {selectedRouter.name}...</p>
            </div>
        );
    }
    ...
    if (error) {
         return (
            <div className="flex flex-col items-center justify-center h-64 bg-slate-800 rounded-lg border border-red-700 p-6 text-center">
                <p className="text-xl font-semibold text-red-400">Failed to load data.</p>
        ...
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-md">
                <div className="p-4 border-b border-slate-700 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <VlanIcon className="w-6 h-6 text-orange-400" />
                        <h3 className="text-lg font-semibold text-orange-400">VLAN Interfaces</h3>
                    </div>
                    <button onClick={() => setIsModalOpen(true)} className="bg-orange-600 hover:bg-orange-500 text-white font-bold py-2 px-3 rounded-lg text-sm">
                        Add VLAN
                    </button>
                </div>
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody>
                            {vlans.length > 0 ? vlans.map(vlan => (
                                <tr key={vlan.id} className="border-b border-slate-700 last:border-b-0 hover:bg-slate-700/50">
                                    <td className="px-6 py-4 font-medium text-slate-200">{vlan.name}</td>
                                    <td className="px-6 py-4 font-mono text-cyan-400">{vlan['vlan-id']}</td>
                                    <td className="px-6 py-4 font-mono text-slate-300">{vlan.interface}</td>
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => handleDeleteVlan(vlan.id)} disabled={isSubmitting} className="p-2 text-slate-400 hover:text-red-500 rounded-md">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-md">
                 <div className="p-4 border-b border-slate-700 flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-semibold text-orange-400">AI Multi-WAN Assistant</h3>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="wanInterfaces" className="block text-sm font-medium text-slate-300">WAN Interfaces</label>
                            <input type="text" name="wanInterfaces" id="wanInterfaces" value={wanInterfaces} onChange={e => setWanInterfaces(e.target.value)} className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white" placeholder="e.g., ether1, ether2, pppoe-out1"/>
                            <p className="text-xs text-slate-500 mt-1">Enter a comma-separated list of your WAN interfaces.</p>
                        </div>
                         <div>
                            <label htmlFor="lanInterface" className="block text-sm font-medium text-slate-300">LAN Interface</label>
                            <select name="lanInterface" id="lanInterface" value={lanInterface} onChange={e => setLanInterface(e.target.value)} required className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white">
                                 {interfaces.filter(i => i.type === 'bridge' || i.type === 'ether').map(i => <option key={i.name} value={i.name}>{i.name}</option>)}
                            </select>
                            <p className="text-xs text-slate-500 mt-1">Select the interface your local users are on.</p>
                        </div>
                        <div>
                            <label htmlFor="wanType" className="block text-sm font-medium text-slate-300">Configuration Type</label>
                            <select name="wanType" id="wanType" value={wanType} onChange={e => setWanType(e.target.value as 'pcc' | 'pbr')} className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white">
                                <option value="pcc">PCC - Load Balance (Merge Speed)</option>
                                <option value="pbr">PBR - Failover</option>
                            </select>
                        </div>
                        <button onClick={handleGenerateWanScript} disabled={isGenerating} className="w-full bg-orange-600 hover:bg-orange-500 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center disabled:opacity-50">
                            {isGenerating ? 'Generating...' : 'Generate Script'}
                        </button>
                    </div>
                    <div className="bg-slate-900 rounded-lg p-2 border border-slate-700 min-h-[300px] relative">
                        {isGenerating && <div className="absolute inset-0 bg-slate-900/80 flex items-center justify-center"><Loader /></div>}

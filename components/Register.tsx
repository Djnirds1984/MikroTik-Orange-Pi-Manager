...
export const Register: React.FC<RegisterProps> = ({ onSwitchToLogin }) => {
    ...
    const { register, error: authError, isLoading, hasUsers } = useAuth();
    ...
    return (
        <div className="w-full max-w-md">
            <h2 className="text-2xl font-bold text-center text-slate-800 dark:text-slate-200 mb-6">
                {hasUsers ? 'Register New User' : 'Create Admin Account'}
            </h2>
             {!hasUsers && <p className="text-center text-sm text-slate-500 dark:text-slate-400 mb-4">Welcome! As the first user, you will be the administrator.</p>}
            <form onSubmit={handleSubmit} className="space-y-4">
             ...
             {hasUsers && <p className="mt-4 text-center text-sm text-slate-600 dark:text-slate-400">

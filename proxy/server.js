...
licenseRouter.get('/status', async (req, res) => {
    ...
    try {
        ...
        if (e instanceof jwt.JsonWebTokenError || e instanceof jwt.TokenExpiredError) {
            ...
            return res.json({ licensed: false, deviceId }); // Fail safely to unlicensed
        }
...
licenseRouter.post('/revoke', protect, async (req, res) => {
    try {
        await db.run("DELETE FROM license WHERE key = 'license_key'");
        res.json({ success: true, message: 'License revoked successfully.' });
    } catch (e) {
        console.error(`[LICENSE] Revoke error: ${e.message}`);
        res.status(500).json({ message: `Revoke error: ${e.message}` });
    }
});
